...
export const deepdivex: Theme = {
  ...
  ui: {
    ...
    idleLoopStartSec: 0.2,  // clip 2 start (the dip hides the exact seam, so this is less critical now)
    idleHandoffDip: true,   // dark-dip the intro->dive hand-off so the position jump happens unseen
    sceneSpeed: { win: 0.75 }, // surface/win clip in slight slow-mo
    sceneZoom: 1.16, // crop the clips' baked-in top/bottom fade (all DEEP DIVE scenes)
